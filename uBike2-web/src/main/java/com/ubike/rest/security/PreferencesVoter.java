...
import com.ubike.rest.service.PrivacyPreferencesResource;
 ...
 * This class is used as a voter for access to the <code>PrivacyPreferencesResource</code>
 * If the requested resource is <code>PrivacyPreferencesResource</code> the vote method
 ...
 * If the requested resource is not <code>PrivacyPreferencesResource</code> so it returns
 * <tt>ACCESS_ABSTAIN</tt>
 *
 * @author Benothman
 */
public class PreferencesVoter extends UbikeResourceAccessVoter {
    ...
    public int vote(Authentication auth, Object secureObj, ConfigAttributeDefinition cad) {
        if (supports(secureObj.getClass())) {
            try {
                ...
                if (mi.getThis().getClass() == PrivacyPreferencesResource.class) {
                    PrivacyPreferencesResource resource = (PrivacyPreferencesResource) mi.getThis();
                    PrivacyPreferences tmp = resource.getEntity();
