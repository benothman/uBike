...
import com.ubike.rest.service.UbikeUsersResource;
 ...
 * This class is used as a voter for access to the <code>UbikeUsersResource</code>
 * If the requested resource is <code>UbikeUsersResource</code> the vote method
 ...
 * If the requested resource is not <code>UbikeUsersResource</code> so it returns
 * <tt>ACCESS_ABSTAIN</tt>
 *
 * @author Benothman
 */
public class UbikeUsersVoter extends UbikeResourceAccessVoter {
    ...
    public int vote(Authentication auth, Object secureObj, ConfigAttributeDefinition config) {
        if (supports(secureObj.getClass())) {
            try {
                ...
                if (mi.getThis().getClass() == UbikeUsersResource.class) {
                    
                    UbikeUsersResource resource = (UbikeUsersResource) mi.getThis();
                    Method method = mi.getMethod();

                    if (method.getName().equals("getUbikeUserResource")) {
                        Object arguments[] = mi.getArguments();
                        Long id = (Long) arguments[0];
                        UbikeUser tmp = resource.getById(id);
