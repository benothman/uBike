...
import com.ubike.rest.service.UbikeGroupsResource;
 ...
 * This class is used as a voter for access to the <code>UbikeGroupsResource</code>
 * If the requested resource is <code>UbikeGroupsResource</code> the vote method
 ...
 * If the requested resource is not <code>UbikeGroupsResource</code> so it returns
 * <tt>ACCESS_ABSTAIN</tt>
 *
 * @author Benothman
 */
public class UbikeGroupsVoter extends UbikeResourceAccessVoter {
    ...
    public int vote(Authentication auth, Object secureObj, ConfigAttributeDefinition config) {
        if (supports(secureObj.getClass())) {
            try {
                ...
                if (mi.getThis().getClass() == UbikeGroupsResource.class) {
                    ...
                    UbikeGroupsResource resource = (UbikeGroupsResource) mi.getThis();
